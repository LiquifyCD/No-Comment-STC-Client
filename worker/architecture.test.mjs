...
test('first view is limited to reader selector and Open control',()=>{assert.match(app,/id="reader"/);assert.match(app,/id="open"/);assert.doesNotMatch(app,/Dina readers|Senaste aktivitet|reader-grid|raw ID|workspace/)});
...
test('single open-door endpoint and public reader aliases are routed',()=>{assert.match(worker,/path==='\/api\/open-door'/);assert.match(worker,/runOpenDoorFlow/);assert.match(worker,/path==='\/api\/reader-options'/)});
test('reader codes and credentials stay out of the browser',()=>{assert.match(config,/READER_CATALOG/);assert.doesNotMatch(config,/PASSAGE_CARD_READER/);assert.doesNotMatch(app,/cardReader|readerCode|access_token|refresh_token|customerId/)});
