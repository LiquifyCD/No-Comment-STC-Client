...
function verifyMutation(request:Request,active:ActiveSession) { if(!sameOrigin(request))return errorResponse('Otillåtet ursprung.',403); const token=request.headers.get('x-csrf-token')??''; if(!token||!safeEqual(token,active.data.csrfToken))return errorResponse('Ogiltig CSRF-token.',403); return null; }
...
async function ensureDefaultReader(env:Env,owner:string) {
 ...
 await env.DB.batch([
  env.DB.prepare('INSERT OR IGNORE INTO readers(id,owner_id,name,name_key,card_reader,created_at,last_opened_at) VALUES(?,?,?,?,?,?,NULL)').bind(id,owner,'Standardläsare','standardläsare',Number(env.PASSAGE_CARD_READER),now),
...
async function createReader(request:Request,env:Env) { const auth=await requireSession(request,env); if('error'in auth)return auth.error; const invalid=verifyMutation(request,auth.active); if(invalid)return invalid; const body=await request.json() as {name?:unknown}; const valid=validateReaderName(body.name); if(!valid.ok)return errorResponse(valid.error,400); const id=crypto.randomUUID(),now=new Date().toISOString(); try{await env.DB.prepare('INSERT INTO readers(id,owner_id,name,name_key,card_reader,created_at,last_opened_at) VALUES(?,?,?,?,?,?,?)').bind(id,auth.owner,valid.name,valid.nameKey,Number(env.PASSAGE_CARD_READER),now,now).run()}catch(error){if(isUniqueError(error))return errorResponse('En läsare med samma namn finns redan.',409);throw error} return response({reader:{id,name:valid.name,createdAt:now,lastOpenedAt:now}},201); }
...
async function passage(request:Request,env:Env,readerId:string) {
 ...
 const auditId=crypto.randomUUID(); const upstream=await fetch(`${env.BRP_BASE_URL}/customers/${encodeURIComponent(result.customerId)}/passagetries`,{method:'POST',headers:authorizedHeaders(auth.active.data),body:JSON.stringify({cardReader:result.cardReader,printTicket:true})}); const outcome=upstream.ok?'accepted':'rejected';
...
export default { async fetch(request:Request,env:Env):Promise<Response> {
 try {
  ...
  const readerMatch=path.match(/^\/api\/readers\/([0-9a-f-]{36})(?:\/(passage))?$/i);
  if(readerMatch){const readerId=readerMatch[1];if(readerMatch[2]==='passage'&&request.method==='POST')return await passage(request,env,readerId);if(!readerMatch[2]&&request.method==='GET')return await getReader(request,env,readerId);if(!readerMatch[2]&&request.method==='PATCH')return await renameReader(request,env,readerId);if(!readerMatch[2]&&request.method==='DELETE')return await deleteReader(request,env,readerId)}
  if(path==='/api/logout'&&request.method==='POST'){const active=await getSession(request,env);if(!active)return response({authenticated:false});const invalid=verifyMutation(request,active);if(invalid)return invalid;return await logout(request,env)}
