...
test('rejects rate-limited requests',()=>assert.equal(validatePassageAttempt({...valid,recentAt:valid.now-999}).status,429));
test('allows another request after one second',()=>assert.equal(validatePassageAttempt({...valid,recentAt:valid.now-1000}).ok,true));
