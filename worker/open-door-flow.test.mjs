...
test('mocked flow logs in, looks up reader, then sends one passage request',async()=>{
  ...
  const resolved=[];
  const result=await runOpenDoorFlow({fetcher,baseUrl:'https://mock.invalid/api',appId:'1',body,resolveReader:async(customerId,readerId)=>{resolved.push({customerId,readerId});return {major:'8251',minor:'543516'}},enabled:true,authorizationId:'APPROVED-mock'});
...
test('invalid credential errors never expose secrets',async()=>{
  ...
  for(const secret of [body.email,body.password,reader,'8251','543516','1306'])assert.equal(serialized.includes(secret),false);
});
