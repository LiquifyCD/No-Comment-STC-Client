...
test('mocked flow logs in, looks up reader, then sends one passage request',async()=>{
  ...
  const fetcher=async(url,init={})=>{
    ...
    if(String(url).includes('/passagereaders?'))return new Response(JSON.stringify({id:1306,name:'Mock reader'}),{status:200,headers:{'content-type':'application/json'}});
    return new Response('{}',{status:200,headers:{'content-type':'application/json','set-cookie':'GCLB=mock; Secure'}});
  };
  const resolved=[];
  const result=await runOpenDoorFlow({fetcher,baseUrl:'https://mock.invalid/api',appId:'1',body,resolveReaderByName:async(customerId,nameKey)=>{resolved.push({customerId,nameKey});return {id:'owned-reader',major:'8251',minor:'543516'}},beforeOpen:async()=>true,enabled:true,authorizationId:'APPROVED-mock'});
  ...
  assert.match(calls[2].url,/passagereaders\?major=8251&minor=543516$/);
  assert.deepEqual(JSON.parse(calls[3].init.body),{cardReader:1306,printTicket:true});
});
...
test('invalid credential errors never expose secrets',async()=>{
  ...
  for(const secret of [body.email,body.password,body.doorName,'8251','543516','1306'])assert.equal(serialized.includes(secret),false);
});
