...
function deviceSessionLine(device){
  const labels={healthy:'Healthy',refresh_pending:'Refresh pending',refresh_failed:'Refresh failed',reauthorization_required:'Reauthorization required'};
  ...
  const refreshed=device.lastRefreshAt?`last refreshed ${new Date(device.lastRefreshAt).toLocaleString()}`:'not refreshed yet';
...
function devicesView(){
  ...
  return `${credential}${ownerStatus}<form id="device-form" class="panel form-panel"><label>Device name<input name="name" value="My iPhone" maxlength="40" required></label><label>Expires after<select name="expiresInDays"><option value="30">30 days</option><option value="60">60 days</option><option value="90">90 days</option><option value="never">Never (credential only)</option></select></label><fieldset><legend>Allowed targets (none means all)</legend>${allowlist||'<p>No targets saved.</p>'}</fieldset><button class="primary" type="submit">Create device credential</button><p id="result" class="result"></p></form><div class="sequence-list">${cards||'<p class="empty-state">No device credentials.</p>'}</div>`;
}
