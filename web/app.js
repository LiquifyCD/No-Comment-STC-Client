...
app.addEventListener('submit',async event=>{
  ...
  if(form.id==='device-form'){const data=new FormData(form),targets=data.getAll('target').map(value=>{const [type,id]=String(value).split(':');return {type,id}});button.disabled=true;try{const created=await request('/api/device-sessions',{method:'POST',headers:{'x-csrf-token':session.csrfToken},body:JSON.stringify({name:data.get('name'),expiresInDays:Number(data.get('expiresInDays')),targets})});oneTimeCredential=created.credential;await loadApp();view();setResult('Device credential created.','success')}catch(cause){setResult(cause.message,'error')}finally{button.disabled=false}return}
