...
function deviceSessionLine(device){
  const labels={healthy:'Healthy',refresh_pending:'Renewal pending',refresh_failed:'Renewal failed',reauthorization_required:'Reauthorization required'};
  const expiry=device.upstreamExpiresAt?`upstream expires ${new Date(device.upstreamExpiresAt).toLocaleString()}`:'upstream expiry unavailable';
  const refreshed=device.lastReauthAt?`last renewed ${new Date(device.lastReauthAt).toLocaleString()}`:device.lastRefreshAt?`last refreshed ${new Date(device.lastRefreshAt).toLocaleString()}`:'not renewed yet';
  return `${labels[device.sessionStatus]||'Reauthorization required'} · ${expiry} · ${refreshed}`;
}
...
function view(){
  ...
  if(!session){app.innerHTML=`<main class="app-shell signed-out"><section class="content login-content"><div class="page-wrap login-wrap"><div class="page-heading"><span class="eyebrow">Welcome</span><h1>Sign in</h1><p>Use your account to manage and open saved doors.</p></div>${loginView()}</div></section></main>`;return}
...
app.addEventListener('click',async event=>{
  ...
  if(event.target.id==='disable-scheduled-reauth'){if(!confirm('Disable automatic renewal and delete the stored encrypted login?'))return;const button=event.target;button.disabled=true;try{await request('/api/scheduled-reauth',{method:'DELETE',headers:{'x-csrf-token':session.csrfToken}});await loadApp();view();setResult('Automatic renewal disabled.','success')}catch(error){setResult(error.message,'error');button.disabled=false}return}
...
app.addEventListener('submit',async event=>{
  ...
  if(form.id==='scheduled-reauth-form'){const data=new FormData(form);button.disabled=true;try{await request('/api/scheduled-reauth',{method:'POST',headers:{'x-csrf-token':session.csrfToken},body:JSON.stringify({username:data.get('username'),password:data.get('password')})});form.reset();await loadSession();await loadApp();view();setResult('Automatic renewal enabled.','success')}catch(cause){setResult(cause.message,'error')}finally{button.disabled=false}return}
