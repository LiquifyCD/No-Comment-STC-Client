...
let session=null,readers=[],sequences=[],defaultSelection=null,selectedTarget='',activeTab='open',loginError='',loadError='',pendingDelete=null,sequenceDraft=null;
...
function appView(){const errorLine=loadError?`<p class="result error">${escapeHtml(loadError)}</p>`:'';if(activeTab==='open')return errorLine+openView();if(activeTab==='create')return errorLine+createDoorView();return errorLine+sequencesView()}

function view(){
  const brand=`<div class="brand"><img src="/icon.svg" alt="" width="34" height="34"><span>No-Comment STC Client</span></div>`;
  ...
  const headings={open:['Open','Choose a saved door or sequence.'],create:['Create a door','Save a custom location and door code.'],sequences:['Sequences','Build ordered door flows with controlled waits.']},[title,description]=headings[activeTab];
  app.innerHTML=`<main class="app-shell authenticated"><header class="app-header">${brand}<div class="session-bar"><span><i aria-hidden="true"></i>Signed in</span><button type="button" id="logout" class="link">Log out</button></div></header><div class="app-body"><nav class="app-nav" aria-label="Main navigation"><button type="button" data-tab="open" class="tab ${activeTab==='open'?'active':''}" ${activeTab==='open'?'aria-current="page"':''}>Open</button><button type="button" data-tab="create" class="tab ${activeTab==='create'?'active':''}" ${activeTab==='create'?'aria-current="page"':''}>Create</button><button type="button" data-tab="sequences" class="tab ${activeTab==='sequences'?'active':''}" ${activeTab==='sequences'?'aria-current="page"':''}>Sequences</button></nav><section class="content"><div class="page-wrap"><div class="page-heading"><span class="eyebrow">Doors</span><h1>${title}</h1><p>${description}</p></div>${appView()}</div></section></div></main><dialog id="delete-dialog"><form method="dialog" id="delete-form"><h2>Delete?</h2><p class="delete-name"></p><p class="delete-error" role="alert"></p><div class="dialog-actions"><button type="submit" value="cancel">Cancel</button><button type="submit" value="confirm" class="danger solid">Delete</button></div></form></dialog>`;
}
...
async function loadApp(){loadError='';try{const [readerData,sequenceData,defaultData]=await Promise.all([request('/api/readers'),request('/api/sequences'),request('/api/default')]);readers=readerData.readers;sequences=sequenceData.sequences;defaultSelection=defaultData.defaultSelection;const preferred=defaultValue();selectedTarget=targetExists(preferred)?preferred:(readers[0]?`door:${readers[0].id}`:sequences[0]?`sequence:${sequences[0].id}`:'')}catch(error){if(error.status===401){session=null;return}readers=[];sequences=[];loadError=error.message}}
async function start(){try{await loadSession()}catch{session=null}if(session)await loadApp();view()}
async function logout(){try{await request('/api/logout',{method:'POST',headers:{'x-csrf-token':session.csrfToken}})}catch{}session=null;readers=[];sequences=[];defaultSelection=null;selectedTarget='';activeTab='open';loginError='';loadError='';pendingDelete=null;sequenceDraft=null;view()}
...
app.addEventListener('click',async event=>{
  const tab=event.target.closest('[data-tab]');if(tab){activeTab=tab.dataset.tab;sequenceDraft=null;view();return}
